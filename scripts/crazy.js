...
jQuery.noConflict()(function ($) {
    ...
    function initialize_map() {
        ...
        $('#map').each(function() {
            ...
            var marker = new google.maps.Marker({
                ...
                map: map,
                tittle: 'The - Crazy',
                icon: '../wp-content/themes/the_crazy/img/marker.png',
	...
	function initIsotop () {
            $('.blog-masonry').isotope('reloadItems');
			$('.blog-masonry').isotope({itemSelector: '.isotope-item'},1);
			$('.crazy-portfolio-list').isotope('reloadItems');
			$('.crazy-portfolio-list').isotope({itemSelector: '.crazy-portfolio-item'},1);
	...
	$(document).on( 'click','#true_loadmore', function() {
		...
		$.ajax({
			...
			success:function(data){
				if( data ) { 
					$('.blog-masonry').append(data);
					$('#true_loadmore').find('span').text('load more');
					current_page++;
	...
	$(document).on( 'click','#true_loadmore_p', function() {
		$(this).find('span').text('Loading...');
		var data = {
			'action': 'loadmore',
			'query': true_posts,
		...
		$.ajax({
			...
			success:function(data){
				if( data ) { 
					$('.crazy-portfolio-list').append(data);
					$('#true_loadmore_p').find('span').text('load more');
					current_page++;
					if (current_page == max_pages) $("#true_loadmore_p").remove();
				} else {
					$('#true_loadmore_p').remove();
				}
				initIsotop();
			}
		});
